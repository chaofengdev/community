 ...
 *
 * 补充：更通俗易懂的解释。
 * 拦截器是Spring MVC框架提供的一种机制，用于在请求处理过程中对请求进行拦截和处理。
 * 可以在请求被处理前、处理中以及处理后执行特定的操作，例如身份验证、日志记录、权限检查等。
 * 拦截器主要是通过实现HandlerInterceptor接口来定义和配置。
 *
 * AOP是一种更为通用的横切关注点处理机制，不仅限于Web请求处理。
 * AOP可以通过在特定连接点上应用通知，将横切关注点模块化并与核心业务逻辑解耦。
 * 在Spring框架中，AOP可以通过AspectJ注解或XML配置来定义和配置切面以及通知。
 * ------------------------------------------------------------------------------------------------------------------
 */
