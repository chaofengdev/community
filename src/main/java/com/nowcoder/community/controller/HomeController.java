...
public class HomeController implements CommunityConstant{
    ...
    public String getIndexPage(Model model, Page page, //前端需要Page对象的相关数据，实现分页效果
                               @RequestParam(name = "orderMode", defaultValue = "0") int orderMode) { //给定默认值为0
        ...
        page.setPath("/index?orderMode=" + orderMode);//page中保存了访问地址

        //查询获得帖子集合   当前起始行号和每页数量都从page实例中获取
        List<DiscussPost> list = discussPostService.findDiscussPosts(0, page.getOffset(), page.getLimit(), orderMode);//重构该方法，使得根据最新或最热排行
